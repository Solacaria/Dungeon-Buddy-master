...
const cleanFilledValues = (role) => (role.includes("~~Filled IMF Spot~~") ? role.slice(0, -1) : role);

const filterSpots = (spots, interactionUserId, reason) => {
    if (reason === "cancelled") {
        return spots.filter((member) => member !== interactionUserId && !member.includes("~~Filled IMF Spot"));
    } else {
        return spots.filter((member) => !member.includes("~~Filled IMF Spot"));
    }
};
...
function parseRolesToTag(difficulty, requiredComposition, guildId) {
    ...
    // Parse the difficulty value (e.g., "M5" becomes 5, "+5" becomes 5)
    const difficultyValue = parseInt(difficulty.replace(/[M+]/g, ""));
